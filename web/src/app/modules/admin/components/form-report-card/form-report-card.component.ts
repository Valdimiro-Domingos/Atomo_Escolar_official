...
@Component({
  selector: 'form-report-card',
  templateUrl: './form-report-card.component.html',
  styleUrls: ['./form-report-card.component.css']
})
export class FormReportCardComponent implements OnInit {

  apiUrl = environmentSettings.endpoint.url
  @Input() lockClose = false;
  @Input() title: string = 'Boletim';
  ...
  constructor(private build : FormBuilder,private router : Router, private http:HttpClient, private setting : DefinitionService,  
  ...
  printPDF(){
    ...
    var dateDue : any = this.reportForm.value?.date_due ?? this.formatDatetimeLocal(new Date());
      const url = `https://vmi1559123.contaboserver.net/escolar/public/api/settings/reports/${this.reportForm.value?.type}/${token?.user?.company_id}/${dateOF}/${dateDue}`;
      window.open(url);
